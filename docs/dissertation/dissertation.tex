...
\begin{document}
    ...
    % How we are using it in our approach
    ...
    \tab
    The first approach is based on color quantization, or simply, reducing the total number of unique
    ...
    \sentence
    This is because that is exactly how the end result SVG will be created, as a set of SVG elements that together
    form the whole image as if they are parts of a partition.
    \sentence
    This way of thinking lends itself extremely well to Connected Component Labelling (CCL) which is basically
    exactly that, dividing the image into a partition of connected components that together form the whole image.
    \sentence
    ...
    component.
    \sentence
    This is obviously very simplistic, but the general concept is the same and the details will be further explained
    in each approach.
    \sentence
