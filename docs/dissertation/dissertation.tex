...
\begin{document}
    ...
    \sentence
    In our case we used a variety of classic automated unit tests, in-code \verb|assert| statements and regular
    ...
    among others.
    \sentence
    Secondly, in-code \verb|assert| statements were used before any requirement that was needed to be true before
    ...
    \sentence
    In addition to the core concepts were used from unit testing such as using in-code \verb|assert| statements
    ...
    \sentence
    An excellent is example is OpenCV's \verb|Mat| type representing a matrix which was used during edge detection
    but the next steps would only know how to handle a built in type representing images.
    \sentence
    Thus, a converter was created to ensure that whenever the \verb|Mat| type was used it was immediately converted
    ...
    \subsubsection{Qualitative Methods}\label{subsubsec:qualitative-methods2}
